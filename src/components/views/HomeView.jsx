...
function HomeView({
  ...
  const recentTracks = ((history && history.length > 0) ? history : trendingTracks).slice(0, 5);
  const recommendedList = trendingTracks.length > 4 ? trendingTracks.slice(2, 6) : SPOTIFY_TRENDING_SONGS.slice(2, 6);
  const album1 = FEATURED_CURATED_MIXES[0] || trendingTracks[0];
  const album2 = FEATURED_CURATED_MIXES[1] || trendingTracks[1];
